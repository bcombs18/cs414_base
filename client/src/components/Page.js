...
export default class Page extends Component {
    ...
    constructor(props) {
        ...
        this.state = {
            showAbout: false,
            serverSettings: {serverPort: getOriginalServerPort(), serverConfig: {filters: null}},
        };

        this.toggleAbout = this.toggleAbout.bind(this);
        this.processServerConfigSuccess = this.processServerConfigSuccess.bind(this);

        sendServerRequest({requestType: "config", requestVersion: 3}, this.state.serverSettings.serverPort)
            .then(config => {
    ...
    renderAtlas() {
        return (
            <Collapse isOpen={!this.state.showAbout}>
                <Atlas createSnackBar={this.props.createSnackBar} serverSettings={this.state.serverSettings}/>
            </Collapse>
        );
    }
