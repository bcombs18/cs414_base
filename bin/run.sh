...
function deploy {
  ...
  if [ -z "$TRAVIS" ]; then
    mvn -f ${REPO_ROOT}/server --global-settings ${REPO_ROOT}/server/.m2/settings.xml clean verify
    check_error $?
  else
    mvn -f ${REPO_ROOT}/server --global-settings ${REPO_ROOT}/server/.m2/settings.xml -DskipTests verify
    check_error $?
  fi
}
...
function run_dev {
  ...
  # Build and Package the JAVA Server
  mvn -f ${REPO_ROOT}/server --global-settings ${REPO_ROOT}/server/.m2/settings.xml clean package
  check_error $?
